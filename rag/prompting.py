...
2) If the answer is not clearly supported by the context, say: "I don't know based on the provided documents."
...
- Include a short "Sources" section listing citations used.
"""
    ...
    return ChatPromptTemplate.from_messages(
        [
            ...
            ("human", 
             ...
             "Return your answer with citations and a Sources section.")
        ]
    )
...
def format_context(docs: List[Dict]) -> str:
    ...
    for i, d in enumerate(docs, start=1):
        ...
        blocks.append(
            f"[{i}] Source: {src} | p.{page} | chunk:{chunk_id}\n"
            f"{d.get('text','')}"
        )
    return "\n\n---\n\n".join(blocks)
